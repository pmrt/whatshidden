...
- Every time you run the process the application will try to restore your previous session
- Your chat logs will be available in the 'chats' directory, in the root path
...
- **`-d, --dumpio`** Show chromium logs

### Available scripts
As you can see there're different ways to initialize the application

- **`npm start`** Will initialize the application in production mode with LOG_LEVEL=info (just the important things)
- **`npm start:verbose`** Will start the application in production mode with LOG_LEVEL=verbose (more detailed logs)
- **`npm start:dev`** Intented just for development. It'll run the application in dev mode, with LOG_LEVEL=verbose and displaying the chromium window (only available in graphical environments)
